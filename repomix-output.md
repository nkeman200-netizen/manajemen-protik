...
database/
  ...
  migrations/
    ...
    2026_08_27_035500_add_read_at_to_warnings_table.php
    2026_08_28_083300_add_type_and_routing_to_documents_table.php
    2026_08_28_212912_add_abbreviation_to_events_table.php
    2026_08_29_175344_add_classification_to_documents_table.php
  seeders/
...
#     steps:
#       - name: Execute Deployment Scripts via SSH
...
#           script: |
#             # Pindah ke direktori proyek di dalam VPS
#             cd /path/ke/folder/manajemen-protik
            
#             # 1. Tarik kode terbaru dari GitHub
#             git pull origin main
            
#             # 2. Pastikan container menyala dan mutakhir
#             docker-compose up -d --build
            
#             # 3. Instal vendor PHP (Mode Produksi: tanpa package testing/dev)
#             docker-compose exec -T app composer install --optimize-autoloader --no-dev
            
#             # 4. Eksekusi migrasi database otomatis
#             docker-compose exec -T app php artisan migrate --force
            
#             # 5. Pastikan jembatan file gambar selalu tersambung
#             docker-compose exec -T app php artisan storage:link
            
#             # 6. Bersihkan dan kompres ulang cache Laravel agar performa maksimal
...
## File: app/Http/Controllers/DocumentGeneratorController.php
````php
<?php
...
## File: database/migrations/2026_08_28_083300_add_type_and_routing_to_documents_table.php
````php
<?php
...
## File: database/migrations/2026_08_28_212912_add_abbreviation_to_events_table.php
````php
<?php
...
## File: database/migrations/2026_08_29_175344_add_classification_to_documents_table.php
````php
<?php
...
return new class extends Migration {
    public function up(): void {
        Schema::table('documents', function (Blueprint $table) {
            $table->string('classification', 100)->nullable()->after('type');
        });
    }
...
server {
    ...
    # Tolak akses ke file tersembunyi seperti .env
    location ~ /\. {
        deny all;
    }
    ...
    location ~ \.php$ {
        ...
        # Izinkan header krusial dari Frontend
...
# ============================================================
# docker-compose.yml — manajemen-protik
# Stack: Laravel 11 + PHP 8.3-FPM + Nginx + MariaDB 10.11
# ============================================================

services:

  # ---------------------------------------------------------
  # Service: app  (Laravel / PHP-FPM)
  # ---------------------------------------------------------
  app:
    build:
      context: .
      dockerfile: Dockerfile
    image: protik-backend-app
    container_name: protik_app
  ...
  db:
    image: mariadb:10.11
    container_name: protik_db
    restart: unless-stopped
    environment:
      MYSQL_DATABASE: ${DB_DATABASE:-protik_db}
      MYSQL_ROOT_PASSWORD: ${DB_PASSWORD:-secret}
      MYSQL_PASSWORD: ${DB_PASSWORD:-secret}
      MYSQL_USER: ${DB_USERNAME:-protik_user}
    ports:
      - "33060:3306"
    volumes:
      - protik_db_data:/var/lib/mysql
    networks:
      - protik_network
...
## File: database/migrations/2026_08_27_035500_add_read_at_to_warnings_table.php
````php
<?php
